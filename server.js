...
function buildExpressionPrompt(opts) {
  ...
  if (mode === "json") {
    const sys = [
      "あなたは、日本の大学入試英語の試験官です。",
      "これから答案文（変数answerNumbered）を評価してください。その際、C-1.文法・語法の正確さ を最優先に評価してください（英語母国語話者としての自然さを問わないが、厳密に）。",
      "次に C-2.語彙の多様性・文構造の多様さ を評価してください（CEFRのB1 レベルを基準）。"
    ].join("\n");

    const c2Policy = [
      "【C-2判定の原則（Bレベル基準・寛容）】",
      "基本は〇（o）を与える。CEFRのB1レベル相当の語彙や複文（because/when/if/that などの従属節・関係詞・to不定詞句 等）が少なくとも時折見られれば十分。",
      "多少の反復や単純文の混在はCEFRのB1レベルへの移行段階として許容する。",
      "ただし、以下の「明確にB1レベル 基準を満たさない極端なケース」 は減点：(1)ほぼすべてが短い単純文の羅列 (2)同一フレーズの反復が支配的で、新しい語彙の導入が極めて少ない (3)等位接続のみで従属結合が皆無。",
      "(1)～(3)に明確に該当 → △/×。",
      "【スコープ制限】内容（主題/論旨/飛躍/矛盾/段落/結論）や談話標識の欠如は扱わない。"
    ].join("\n");
  ...
  if (mode === "detail") {
    ...
    const rules = [
      ...
      "《改善ポイント》に含む必須情報：文番号→項目番号順、改善すべき表現、修正例（過剰な意味改変は避け、実文に即した具体性を重視）、修正理由（学習者にとってわかりやすく）。",
      "《改善ポイント》については、必須情報があれば、その他の出力形式の制約は設けない。箇条書き・段落・例文の提示など、あなたの判断で自由に構成。",
    ].join("\n");

    return [
      "日本の大学入試英語を教えている、生徒のやる気を引き出す予備校の先生です。",
...
function buildContentPrompt(opts) {
  ...
  if (mode === "json") {
    const sys = [
      ...
      "「論理の飛躍(leaps)」や「論理の矛盾（順接・逆接など）」は誰が読んでも明らかな場合のみ数えてください。明らかな談話標識の欠如は減点してください。",
      "評価項目：2-1主題の一貫性 / 2-2本文の論理展開。",
      "【スコープ制限】文法・語法や文構造（主語と動詞の一致｜倒置）を扱わない。",
  ...
  if (mode === "detail") {
    ...
    const rules = [
      ...
      "《改善ポイント》には△や×と判断した項目に関するものをすべて列挙。",
      "《改善ポイント》に含む必須情報：文番号→項目番号順、改善すべき表現、修正例（過剰な意味改変は避け、実文に即した具体性を重視）、修正理由（添削対象者にとってわかりやすく）。",
      "《改善ポイント》については、必須情報があれば、その他の出力形式の制約は設けない。箇条書き・段落・例文の提示など、あなたの判断で自由に構成。",
